"""03b_optuna_search2.py: second Optuna hyperparameter search (Appendix B).
...
*** HPC-GRADE *** Trains one Planck-RACS NRE per trial (369 in the paper, over
several jobs sharing an SQLite study) on 4 GPUs for up to 48 hours per job
(see pbs/optuna_search.pbs).

With one filter, searches over the number of dense layers and neurons, the dropout
rate, the initial learning rate and the activation function (ReLU, LeakyReLU,
GELU, ELU), training each NRE for at most 10 epochs on 100 000 simulation pairs
(patience 10). The objective is the KL divergence between the predicted and the
nested sampling (from 02c) in concordance log R distributions. The study is saved
to outputs/optuna/search2/optuna_study.db and visualised at the end.
...
metric_of_interest = 'KL_divergence'
# metric_of_interest = 'KS_statistic'
# metric_of_interest = 'KS_pvalue'
print(f'Using {metric_of_interest} as the metric of interest.')

if metric_of_interest == 'R2_score' or metric_of_interest == 'KS_pvalue':
    direction = "maximize"
else:
    direction = "minimize"
...
parser.add_argument('--nsamples', type=int, default=100000, help='simulation pairs used by this search (paper: 100000)')
parser.add_argument('--nchunks', type=int, default=10, help='chunks they are split into (paper: 10)')
parser.add_argument('--full-nchunks', type=int, default=config.N_CHUNKS, help='--nchunks used for 01a_make_simulations.py')
parser.add_argument('--epochs', type=int, default=10, help='maximum epochs per trial (paper: 10)')
parser.add_argument('--n-trials', type=int, default=200, help='number of trials in this job')
...
def objective(trial):
    ...
    neurons = trial.suggest_categorical('neurons', [64, 128, 256, 384, 512])
    dropout_rate = trial.suggest_float('dropout_rate', 0.0, 0.20, step=0.1)
    learning_rate = trial.suggest_float('learning_rate', 1e-4, 1e-2, log=True)
    activation_function = trial.suggest_categorical('activation_function', ['relu', 'leakyrelu', 'gelu', 'elu'])

    NETWORK_DIR = BASE_DIR + f'layers{layers}_neurons{neurons}_dropout{dropout_rate:.2f}_learning_rate{learning_rate:.2e}_activation{activation_function}/'
    ...
    kl_str = f"{kl:.6e}"
    print(f'KL divergence for {NETWORK_DIR}:', kl)
    np.save(NETWORK_DIR + 'KL_divergence.npy', kl)
    ...
    print(f'KS statistic for {NETWORK_DIR}:', ks_stat)
    ks_pvalue_str = f"{ks_pvalue:.6e}"
    print(f'KS p-value for {NETWORK_DIR}:', ks_pvalue)
    np.save(NETWORK_DIR + 'KS_statistic.npy', ks_stat)
    np.save(NETWORK_DIR + 'KS_pvalue.npy', ks_pvalue)
    ...
    return np.load(NETWORK_DIR + f'{metric_of_interest}.npy')

# Use SQLite storage so the study is persisted to disk
db_path = os.path.abspath(os.path.join(BASE_DIR, "optuna_study.db"))
storage = f"sqlite:///{db_path}"
study_name = f"tensionnet_{search}"

study = optuna.create_study(storage=storage, study_name=study_name, direction=direction, load_if_exists=True)
...
if metric_of_interest == 'KL_divergence':
    metric_of_interest = 'logKL_divergence'
VIS_DIR = BASE_DIR + f'visualisations/{metric_of_interest}/'
if not os.path.exists(VIS_DIR):
    os.makedirs(VIS_DIR)

fig = optuna.visualization.plot_slice(study, target_name=metric_of_interest)
pio.write_image(fig, VIS_DIR + 'optuna_slice_plot.pdf')

fig = optuna.visualization.plot_param_importances(study, target_name=metric_of_interest)
pio.write_image(fig, VIS_DIR + 'optuna_param_importances.pdf')

fig = optuna.visualization.plot_contour(study, params=["layers", "neurons", "dropout_rate", "learning_rate", "activation_function"], target_name=metric_of_interest)
pio.write_image(fig, VIS_DIR + 'optuna_contour_plot.pdf')

fig = optuna.visualization.plot_parallel_coordinate(study, target_name=metric_of_interest)
pio.write_image(fig, VIS_DIR + 'optuna_parallel_coordinate_plot.pdf')